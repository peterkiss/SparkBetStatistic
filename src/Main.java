...
public class Main {
	...
	public static void main(String[] args) {
		...
		System.out.println("3. Get runner ids");
		while(!ans){
			System.out.println("Ans =");
			choose = in.nextInt();
			ans  = choose == 1 || choose == 2|| choose == 3;
		}
		if(choose == 1 )
			allSport(textFile);
		else if(choose == 2 )
			...
			filterLines(textFile);
	}
	private static void filterLines(JavaRDD<String> textFile) {
		try {
			PrintWriter w = new PrintWriter(new File("result.csv"));

			System.out.println("Filters : ");
			Scanner sc = new Scanner(System.in);
			String filters = sc.nextLine();
			String[] fa = filters.split(" ");
			//JavaRDD<String[]> result =
			List<String> result =
					textFile.filter(new Function<String, Boolean>() {
						public Boolean call(String s) { 
							Boolean res = true;
							for(int i = 0; i<fa.length && res;++i){
								res &= s.contains(fa[i]);
							}
							if(res)
					...
					}).map(line->line.split(",")).mapToPair(s -> new Tuple2<String , String[]>(s[0],s)).reduceByKey((x,y)->x).values().map(s->Arrays.toString(s)).collect();

			//		result.foreach(s -> w.println(s));
			for(String s : result)
				w.println(s);
			w.close();
